...
class McpMockService {
  ...
  private getAds(args?: any): AdCreative[] {
    ...
    return names.map((name, i) => {
      ...
      const conversions = sales;
      const checkouts = landingClicks;
      ...
      return {
        ...
        checkouts,
        cost_per_checkout: checkouts > 0 ? Math.round((spend / checkouts) * 100) / 100 : 0,
        profit,
        revenue,
        landing_views: landingViews,
        cic: landingViews > 0 ? Math.round((spend / landingViews) * 100) / 100 : 0,
        ctr: Math.round((clicks / impressions) * 10000) / 100,
        play_rate: Math.round((videoViews / impressions) * 10000) / 100,
        hook_rate: Math.round((5 + Math.random() * 40) * 100) / 100,
        ...
        quality_ranking: ['HIGH', 'MEDIUM', 'LOW'][Math.floor(Math.random() * 3)],
        creative_conversion_rate: Math.round((2 + Math.random() * 10) * 100) / 100,
