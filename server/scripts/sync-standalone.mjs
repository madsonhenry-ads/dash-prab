/**
 * Sync EasyTracker → Railway (standalone)
 *
 * Roda OAuth, busca dados e POST para o Railway em um único processo.
 * O token fica armazenado em memória (não usa DPAPI).
 *
 * Uso:
...
async function main() {
  ...
  console.log('O navegador vai abrir para autorizacao. Faca login e autorize.\n');
  ...
  // Connect - SDK lida com dynamic client registration + OAuth flow
  ...
  const tools = [
    ...
    ['Campanhas', 'easytracker_list_campaigns', {}, (r, d) => {
      if (r.content?.[0]?.text) d.campaigns = JSON.parse(r.content[0].text);
    }],
    ['Ad Sets', 'easytracker_list_ad_sets', {}, (r, d) => {
      if (r.content?.[0]?.text) d.adSets = JSON.parse(r.content[0].text);
    }],
    ['Criativos', 'easytracker_list_ads', { period: 'today' }, (r, d) => {
      if (r.content?.[0]?.text) d.ads = d.creatives = JSON.parse(r.content[0].text);
    }],
    ['Contas', 'easytracker_list_ad_accounts', {}, (r, d) => {
      if (r.content?.[0]?.text) d.adAccounts = JSON.parse(r.content[0].text);
    }],
    ['Produtos', 'easytracker_list_offers', {}, (r, d) => {
      if (r.content?.[0]?.text) d.products = JSON.parse(r.content[0].text);
    }],
    ['Canais', 'easytracker_list_traffic_channels', {}, (r, d) => {
      if (r.content?.[0]?.text) d.trafficChannels = JSON.parse(r.content[0].text);
    }],
  ];
